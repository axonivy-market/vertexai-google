...
public class GeminiDataRequestService {
  ...
  public String getAccessToken() throws IOException {
    GoogleCredentials credentials = ServiceAccountCredentials
        .fromStream(GeminiDataRequestServiceUtils.getInputStream(VERTEX_KEY_FILE_PATH)).createScoped(vertexAiScopes);
  ...
  public List<Conversation> sendRequestToGemini(String message, Model platFormModel)
    ...
    HttpResponse<String> response = sendRequest(message, platFormModel);
    if (response.statusCode() == 200) {
      ...
      Content contentResponse = Optional.ofNullable(responseRoot).map(ResponseRoot::getCandidates)
          .map(Collection::stream).flatMap(Stream::findFirst).map(Candidate::getContent).map(Content::getParts)
          .map(Collection::stream).flatMap(Stream::findFirst).map(Part::getText).map(text -> {
  ...
  public HttpRequest generateHttpRequestBasedOnModel(Model platformModel, String bodyRequestContent)
    ...
    return HttpRequest.newBuilder().uri(URI.create(geminiEndpoint)).header(CONTENT_TYPE_HEADER, APPLICATION_JSON_TYPE)
        .POST(HttpRequest.BodyPublishers.ofString(bodyRequestContent)).build();
  }

  public HttpResponse<String> sendRequest(String message, Model platFormModel)
      throws IOException, InterruptedException {
